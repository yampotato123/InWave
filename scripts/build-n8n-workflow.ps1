...
// 條件段 A2：使用者替歌單取了名字才附加
// 放在情緒之後、濾鏡之前，因為名字是最強的意圖訊號——「夏天的海邊」這種
// 一句話，比八個情緒詞加三個滑桿加起來都準。但仍要求以照片為主體，
// 否則名字會蓋掉照片本身的內容，變成純文字推薦。
if (b.playlistName) {
  p += `

使用者把這份歌單命名為「${b.playlistName}」。這個名字是他想要的氣氛的直接表達,
請讓推薦貼近這個名字所描述的情境,但仍要與照片內容相符——名字是方向,照片是主體。`;
}
