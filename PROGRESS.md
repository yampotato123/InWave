...
## 更新（2026-08-19）：階段 3 步驟 5 完成 → **階段 3 結束**

AI 掛掉時功能不會消失。回落有兩層，都經過實測（不是只看程式碼推論）：

```
第一層  AI 判讀失敗(NETWORK / TIMEOUT / HTTP_xxx / AI_UNAVAILABLE / BAD_JSON)
        → 不寫入資料庫,改用 MoodKeywordMapper 的規則關鍵字
第二層  AI 判讀成功,但推的歌在 YouTube 一首都找不到
        → 同樣回落關鍵字搜尋,並記 warning
```
...
| 情境 | 結果 |
|---|---|
| n8n 停著 + 全新照片（無判讀紀錄）| HTTP **200**，11 秒，log：`呼叫 n8n 失敗` → `AI 判讀失敗(photoId=4):NETWORK` |
| n8n 停著 + 已有判讀紀錄的照片 | HTTP **200**，2997 ms，完全不需要 n8n |
| n8n 開回來後重試同一張照片 | `AI 判讀成功並已存檔(photoId=4,model=models/gemini-3.6-flash)` |
| 再開一次 | `已有 AI 判讀(photoId=4,12:24),不重複呼叫`，2893 ms |

**「判讀失敗不寫入」的決定被這組實測驗證了**：photoId=4 第一次失敗沒有留下紀錄，
所以 n8n 回來後會自動重試成功。若當初寫了一筆空判讀，這張照片就永遠卡住。

測試 **44/44**（新增逾時與連線失敗兩項——這兩條路先前只有程式碼，沒有斷言守著）。

### 一個沒有修的已知代價

**n8n 停著時，全新照片的推薦頁要等約 11 秒**（連線被拒之前的等待）。
不是掛住，只是慢。真要壓下來就在 `Program.cs` 的 typed client 加
`SocketsHttpHandler { ConnectTimeout = TimeSpan.FromSeconds(5) }`——
但那只在「n8n 掛掉」時有感，正常情況毫無差別，所以先不動。

---

## 階段 3 總結（0819 完成）

推薦鏈從「8 情緒 × 4 濾鏡 = 32 種固定關鍵字」變成「AI 看照片推薦真實歌曲」。

```
上傳照片 → (可不選情緒) → 修圖
    → n8n/Gemini 判讀氣氛並推薦 5 首  ← 判讀結果持久化,一張照片只判一次
    → 逐首在 YouTube 找到那支影片      ← 找不到的快取起來,不重複燒配額
    → 存成歌單
```
...
其中兩個會造成資料遺失或整頁 500。這一節值得留著——它們的共通點是
「正常情況都對，壞在外部服務不照約定的時候」。

### Critical

**1. 重新判讀先毀後建 → 資料永久遺失**（`WorksController.Reanalyze`）

舊版先清情緒、先刪舊判讀，**然後**才去問 AI——而 AI 會失敗。當天 Gemini 連續
一小時回 503，按下「重新判讀」的結果就是：情緒變空、書背變灰、舊判讀也救不回來。

改為**先做完新判讀，成功才覆蓋**；失敗則什麼都不動，並在畫面上說明。
實測（Gemini 仍 503 時按下重新判讀）：判讀時間仍是 `08/19 20:35`、情緒仍是「懷舊」，
畫面顯示「重新判讀沒有成功（AI_UNAVAILABLE），先前的判讀結果保留著」。

**2. 併發判讀撞唯一索引 → 500**（`EnsureAnalysisAsync`）

「先查沒有、再插入」之間隔著 16–105 秒。使用者等太久按 F5、或開兩個分頁，
兩個請求都會看到 null 然後都插入，第二個撞上 `IX_PhotoAnalyses_PhotoId`。
改為 catch `DbUpdateException` → 重讀既有那筆繼續用（不是錯誤狀態，是別人先存好了）。

### 我自己種下的因

**3.「MoodName 等於上次 moodPick 就是 AI 填的」這個推論是錯的**

prompt 明寫「使用者有指定情緒時，`moodPick` 直接回傳該值」——所以**使用者自己選的
情緒必然等於上次的判讀結果**。那個判斷式會把使用者的選擇全部誤判成 AI 填的，
正好與它宣稱要保護的東西相反。

改成明確記錄來源：`MoodProfile.IsAiFilled`（一次 migration）。**不做事後推論。**

**4. n8n 的 `{ ok: true, ...JSON.parse(cleaned) }`——展開在後，AI 回一個 `ok` 就能覆寫成功旗標**

最不可信的輸入決定了成敗判準。改成 `{ ...JSON.parse(cleaned), ok: true, model: ... }`。

### 其餘

**5. 型別不符會丟未捕捉例外**，違反 `PhotoAnalysisService` 對外宣告的「不丟例外」。
最外層是陣列、`ok` 是字串、`error` 是數字——這些都會炸成 500 而不是回落。
已改為型別先驗再取，並補 14 項測試涵蓋每一種壞法。

**6. `songs` 沒有上限**：模型漂移回 40 首 = 一次 4,000 單位的 YouTube 配額。已加 `Take(5)`。

**7. `Pick` 的 Topic 規則凌駕歌手比對**：翻唱歌手也有自己的「- Topic」頻道。
改為「Topic 且頻道含歌手名 → 含歌手名 → 任何 Topic → 第一筆」。

順帶：`JsonDocument` 補 `using`；YouTube 快取鍵轉小寫（`Beach House` 與 `beach house`
先前會各燒一次配額）；`SavePlaylist` 驗證失敗重繪時補回 `AnalyzedAt`（否則按鈕整組消失）；
「重新判讀」按鈕送出後停用。

測試 **58/58**（原 44 + 新增 14）。**型別不符那幾條在修之前是會丟例外的**，不是補形式。

### 沒驗到的一格

「重新判讀 → 立刻成功」仍沒看到：Gemini 從 21:00 起持續 503 超過一小時。
那條路與已驗證多次的「首次判讀成功」是同一段程式碼。

---

### 收尾二：三個接縫補完（2026-08-19）

| 接縫 | 做法 | 實測 |
|---|---|---|
| 重新判讀入口 | `POST /Works/Reanalyze` 刪掉舊 `PhotoAnalysis` 後轉址回推薦頁 | log `已清除 AI 判讀(photoId=6)`，n8n 執行 16→17 |
| 容器看不到真推薦 | `.env` 填入 YouTube 金鑰（與本機 user-secrets 同一把）| 容器內推薦變成真實影片，不再是 `[示範]` |
| n8n 停著時等 11 秒 | typed client 加 `SocketsHttpHandler.ConnectTimeout = 5s` | 11 秒 → **8 秒**（其餘是本來就有的工作＋真實 YouTube 搜尋）|

**「重新判讀」的情緒處理**：若 `MoodName` 與上次判讀的 `moodPick` 相同，視為 AI 填的，
一併清空讓新判讀能重填；使用者自己選的則不動。剛好撞值時清掉再填回同一個值，沒有副作用。

~~**沒驗到的一格**：「重新判讀 → 立刻成功」~~ → **2026-08-20 已補驗**（換 OpenAI 之後）：

```
判讀時間  08/20 00:43 → 08/20 11:22          ← 確實被覆蓋
log       AI 判讀成功並「已覆蓋舊判讀」(photoId=9,model=gpt-4o)
再開一次  已有 AI 判讀,不重複呼叫
耗時      16 秒
```

`已覆蓋舊判讀` 這句只會出現在 `Reanalyze` 的成功路徑（首次判讀是「已存檔」），
所以走到的確實是那條路，不是被首次判讀的程式碼矇混過去。

當時沒驗到的原因記著：2026-08-19 21:00 起 Gemini 持續回 503 `high demand`，
連試六次都是（每次頁面仍正常回 200，等於又驗了一次回落）。
...
- ~~沒有「重新判讀」的入口~~ → 2026-08-19 已補（見「收尾二」）
- ~~容器的 `.env` 沒有 `YOUTUBE_API_KEY`~~ → 2026-08-19 已填
- **階段 4（AI 風格濾鏡）未開始** —— 本來就是選配，使用者 2026-08-19 決定不做
- **上架前必辦**：`wwwroot/uploads/` 換成物件儲存（見「已知限制」第一條）
- **轉公開前必辦**：commit 歷史含他人公司信箱（見「資安檢查」）
...
### ~~下一步：階段 3，AI 判讀照片~~ → 2026-08-19 已完成（見本檔最上方「階段 3 總結」）

當時的判斷後來都成立：接入點確實在 `Recommend`、持久化確實必要
（沒有它每次重新整理都是一次付費呼叫），也確實只需要一次 migration。
...
| **3-5　fallback 與測試** | ✅ **完成並實測**（2026-08-19，真的停掉 n8n 驗過）|
| **階段 3 整體** | ✅ **完成** |
| 4　AI 風格濾鏡（選配） | ⬜ 未開始 |
